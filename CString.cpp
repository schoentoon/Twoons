...
CString CString::HexToHtml(CString c)
{
  int x;
  stringstream s;
  s << c;
  s >> std::hex >> x;
  return CString("&#" + CString(x) + ";");
}

unsigned int CString::HexToInt(CString c)
{
  unsigned int x;
  stringstream s;
  s << c;
...
CString CString::IntToHex(int i)
{
  stringstream s;
  s << std::hex << i;
  return s.str();
...
CString CString::ConvertUnicode(CString input, bool emoji)
{
  for (unsigned int i = 0; i < input.size(); i++) {
    try {
      if (input.substr(i,2).compare("\\u") == 0) {
        if (!emoji)
          input.Replace(input.substr(i,6), HexToHtml(input.substr(i+2,4)));
        else {
          unsigned int hexCode = HexToInt(input.substr(i+2,4));
          if (hexCode >= 57345 && hexCode <= 58679)
            input.Replace(input.substr(i,6), "<img src=\"img/emoji/" + input.substr(i+2,4) + ".png\"/>");
          else
            input.Replace(input.substr(i,6), HexToHtml(input.substr(i+2,4)));
        }
      }
